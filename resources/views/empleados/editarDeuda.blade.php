...
.no-spinners::-webkit-inner-spin-button {
  ...
  margin: 0;
	...
	<form id="formCreate" method="POST" action="{{URL('/empleado/insertarDeuda')}}" enctype="multipart/form-data">
		...
		{{csrf_field()}}
		<input type="number" name="empleado_id" value="{{ $deuda->empleado->id }}" hidden>
		<input type="number" name="deuda_id" value="{{ $deuda->id }}" hidden>
		<h4>Dude de empleado {{ $deuda->empleado->nombre.' '.$deuda->empleado->apellido1.' '.$deuda->empleado->apellido2 }}</h4>
		<div class="form-group">
			<div class="form-row">
				<div class="col-5">
					<label >Monto: </label>
				    <input name="monto" class="form-control no-spinners" type="number" onkeypress="return isNumberKey(event)" placeholder="Monto en pesos" value="{{ $deuda->monto }}">
				    @if($errors->any())
			...
			<div class="form-row">
			    <div class="col-12	">
					...
					<select class="form-control custom-select" name="tipo">
			          	<option value="{{ $deuda->tipo }}" selected="true">{{ $deuda->tipo }}</option>
			...
			<div class="form-row">
			    <div class="col-12	">
					...
					<select class="form-control custom-select" name="plazo">
			          	<option value="{{ $deuda->plazo }}" selected="true">{{ $deuda->plazo }}</option>
			  ...
			  <div class="modal-dialog" role="document">
			    <div class="modal-content">
			      ...
			      <div class="modal-footer">
			        ...
			        <a href="{{URL('empleado/'.$deuda->empleado_id.'/perfil')}}" class="btn btn-outline-danger">Salir sin Guardar</a>
			      </div>
			    </div>
			  </div>
			</div>
